...
@Deprecated
@NoArgsConstructor
public class OldPodcastPackageManifest {
	...
	@SneakyThrows
	public static OldPodcastPackageManifest from(File manifest) {
	...
	@SneakyThrows
	private static OldPodcastPackageManifest from(InputStream inputStream) {

		var dbf = DocumentBuilderFactory.newInstance();
		var db = dbf.newDocumentBuilder();
		var doc = db.parse(inputStream);
		var build = new OldPodcastPackageManifest();
	...
	@SneakyThrows
	public static OldPodcastPackageManifest from(String manifestContents) {
		try (var in = new ByteArrayInputStream(
				manifestContents.getBytes(StandardCharsets.UTF_8))) {
			return from(in);
		}
	}
