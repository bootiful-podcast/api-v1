...
class FilesToRequestsFlowConfiguration {
	...
	FilesToRequestsFlowConfiguration(RabbitHelper helper, ObjectMapper om,
		...
		helper.defineDestination(properties.getProcessor().getRequestsExchange(),
				properties.getProcessor().getRequestsQueue(),
				properties.getProcessor().getRequestsRoutingKey());
	}

	@Bean
	IntegrationFlow filesToRequestsFlow(RabbitRequestsFlowConfiguration requestsFlow) {

		var file = this.properties.getProcessor().getInboundPodcastsDirectory();
